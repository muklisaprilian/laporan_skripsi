<?php	
	...
	$this->fpdf->SetFont('arial','B',10);
	$this->fpdf->Ln(0.3);
	$this->fpdf->Cell(0,1,'LAPORAN DATA MATERI UJIAN',0,0,'C');
	
	$this->fpdf->SetLineWidth(0);
	$this->fpdf->SetFont('arial','',9);
	$this->fpdf->Ln(1.3);
	$this->fpdf->Cell(3.5,0.5,'Kode Materi Ujian',0,0,'L');				
	$this->fpdf->Cell(3.5,0.5,' :  '.$id_materi,0,0,'L');		

	$this->fpdf->Ln(0.5);
	$this->fpdf->Cell(3.5,0.5,'Jalur Tes',0,0,'L');		
	$this->fpdf->Cell(3.5,0.5,' :  '.$jalur_tes,0,0,'L');		

	$this->fpdf->Ln(0.5);
	$this->fpdf->Cell(3.5,0.5,'Jumlah Soal',0,0,'L');
	$this->fpdf->Cell(3.5,0.5,' :  '.$j_soal,0,0,'L');

	$this->fpdf->Ln(0.5);
	$this->fpdf->Cell(3.5,0.5,'Durasi',0,0,'L');
	$this->fpdf->Cell(3.5,0.5,' :  '.$durasi,0,0,'L');
